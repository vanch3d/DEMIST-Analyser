...
BOOL CDocFrameMgr::OnCmdMsg( UINT _nID, int _nCode, void* _pExtra )
{
    ...
    else if ( _nCode == CN_UPDATE_COMMAND_UI && pChosenFrame )
    {
        ...
        ( ( CCmdUI* ) _pExtra )->Enable(  TRUE);//bEnCmd);
...
CFrameWnd* CFrame::FindFrameWnd( void )
{
    ...
    while ( Position != NULL )
    {
		...
		CRuntimeClass *pRTV = pView->GetRuntimeClass();
		CRuntimeClass *pRTP = pView->GetParentFrame()->GetRuntimeClass();
		...
		if (pRTV == m_pFrameTemplate->GetViewRTC() &&
			pRTP == m_pFrameTemplate->GetFrameRTC())
        {
            // Got it.
