...
class CFrameTemplate
{
    ...
    private:
		...
		BOOL m_bMultipleView;			// NVL : to allow multiple different view
	...
	public:
			...
			BOOL _bMultipleView = FALSE)
		{
			...
			m_bMultipleView		= _bMultipleView;
		}	
		...
		// NVL
		BOOL GetMultipleView( void )			{ return m_bMultipleView; };
		void SetFrameRTC(CRuntimeClass* pRTC)	 	{ m_pFrameClass = pRTC; };
		void GetViewRTC(CRuntimeClass* pRTC)		{ m_pViewClass = pRTC; };

		void SetLoadAtStartup(BOOL bLoad)		{ m_bLoadAtStartup = bLoad; };
...
class CFrame : public CMultiDocTemplate
{
    ...
    public:
		...
		BOOL GetMultipleView( void )		 	{ return m_pFrameTemplate->GetMultipleView(); };
		CDocument* GetDocument( void )	 	{ return m_pDocument; };
    ...
    public:
        ...
        bool ShowOrCreateFrame( bool bIsOpen);         // To display the frame window.
